...
export interface GlobalDateRangeController {
  dateRange: DateRangeStrings | null;
  setDateRange: (range: DateRangeStrings) => void;
  clear: () => void;
}

export function GlobalDateRangeControl({
  controller,
  className = '',
  emptyLabel = 'Periodo da carteira',
  emptyDetail = 'Aguardando dados',
}: {
  controller: GlobalDateRangeController;
  className?: string;
  emptyLabel?: string;
  emptyDetail?: string;
}) {
  const { dateRange, setDateRange, clear } = controller;
  ...
  const displayLabel = dateRange ? getDateRangeLabel(dateRange) : emptyLabel;
          ...
          <div className="flex items-center justify-between border-t border-border/50 bg-muted/20 px-5 py-3">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] text-muted-foreground">
                {dateRange ? `${formatDateDisplay(dateRange.from)} - ${formatDateDisplay(dateRange.to)}` : emptyDetail}
              </span>
            </div>
